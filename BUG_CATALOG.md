...
| #151 | pytest collection крашился на Windows `ValueError: I/O operation on closed file` | `Scripts/arb_server.py:25` безусловно оборачивал `sys.stdout` в `io.TextIOWrapper` при импорте, тем самым уводя buffer pytest's tmpfile. Также `StreamHandler()` без аргументов привязывался к sys.stderr на import-time | Wrap под `if 'pytest' not in argv[0] + hasattr(.buffer)`; StreamHandler install только если NOT under pytest. **864 теста собираются** (было 7 + краш) |
| #151 | Cross-platform fee math (Phase 19v34) — 0 unit tests | После добавления `to_radar_deal_format` с fee/gross/roi/adj_roi math — никаких golden tests. Реальные баги всплыли бы только в проде | +7 edge-case тестов (stake clip, slip cap, theta selection across Poly×Lim / SX×Lim pairs). 13 → 20 tests, all green |
| #151 | analytics_events.jsonl содержит ТОЛЬКО `opened`/`closed` для HOT pool; NEAR drift'ы не записаны | `_snapshot()` вызывается только в `update_from_scan(deals)` (= active arbs). NEAR pool — отдельная вселенная, в analytics не попадает | `update_from_near_scan()` + `_near_snapshot()` + wiring в `analytics_loop()`. `aggregate()` возвращает новый `near` funnel (count + by_platform + by_structure + near_to_arb_ratio) |
| TBD (post-#151) | **BUG-D1 (phantom import)**: `Scripts/arb_server.py:6480` импортирует `from poly_l2_http import l2_request` — модуль НЕ существует в репо | `poly_l2_http.py` остался в stash на branch `feature/ts-5a-real-http-fires` (Phase 19v25, 05.05.2026) и никогда не был смержён в main. Под dry-run без poly creds — silent dead code. С poly creds в real-mode — ImportError на `_make_poly_fetcher` → весь reconciliation layer падает silently | Wrap import в `try/except ImportError` + return None из `_make_poly_fetcher`. Caller (`for w in _wallet_pool.wallets`) пропускает None. Polymarket position reconcile временно disabled, fill-confirmation via WS остаётся primary detection. До-фиксить — портировать `poly_l2_http.py` из stash или переписать L2 HMAC на `poly_l2.py` module |
...
- **Безусловное** оборачивание `sys.stdout` в `TextIOWrapper` или установка `StreamHandler()` на module-level → pytest's I/O capture ломается на Windows. Всегда гейтить через `'pytest' not in argv[0]` + `hasattr(sys.stdout, 'buffer')`.
- **Невидимые funnel'ы.** Если что-то фильтруется/преобразуется в пайплайне (NEAR pool, cross-platform pairing, paper graduation), всегда должен быть endpoint показывающий per-step counts — иначе оператор смотрит на "0 сделок" и не знает в каком из 5 шагов дропнуло.
...
| #58 | Phase 16: maker wire-up в fire_arb + Q1 adaptive multi-outcome bot relaxation + Q2 SX type expansion + Limitless revert flow | maker mode не был wired, multi-outcome events с N>6 отказывались, SX узкая type whitelist, Limitless revert TODO | _fire_one_leg_maker path, maker_then_taker fallback, N-aware override (force taker для N≥4), adaptive _assign_wallets (1/2/3 legs per bot tiers 6/12/13+), SX expanded to 36 types, Limitless SELL FOK revert path |
| #58 | Phase 16+: settlement timing + per-chain balance + Limitless CTF approve | оператор-flagged upcomings | settlement_timing_check 24h tolerance + PER_CHAIN_CONFIG + check_balance_for_platform + limitless_approve.py CTF setApprovalForAll |
| #58 | Phase 17 (final): SX EIP-712 signing + SX revert opposite-outcome + 3-way 1X2 stub + fetch_limitless_positions + filter_wallets_by_chain | оставшиеся блокеры real-mode | _sign_sx_order_fill (chainId 4162, Details type), SX revert via opposite-outcome taker fill, eval_sx_3way stub, fetch_limitless_positions через X-API-Key, register_limitless_fetcher / register_sx_fetcher, filter_wallets_by_chain |
